...
test_that("multivariate data routes to NormalMvSpec, univariate to NormalUvSpec", {
  expect_s4_class(Rnimix:::.selectClusterSpec("normal", TRUE,  2L), "NormalMvSpec")
  expect_s4_class(Rnimix:::.selectClusterSpec("normal", FALSE, 1L), "NormalUvSpec")
  # forcing the wrong family for the data shape errors clearly
  expect_error(Rnimix:::.selectClusterSpec("normal-mv", FALSE, 1L), "matrix")
  expect_error(Rnimix:::.selectClusterSpec("normal-uv", TRUE,  2L), "vector")
  # Student-t (v0.4.0) now routes to the univariate / multivariate spec.
  expect_s4_class(Rnimix:::.selectClusterSpec("student-t", FALSE, 1L), "StudentTUvSpec")
  expect_s4_class(Rnimix:::.selectClusterSpec("studentt",  TRUE,  2L), "StudentTMvSpec")
  # a family that is genuinely not provided still errors clearly
  expect_error(Rnimix:::.selectClusterSpec("gamma", FALSE, 1L), "not available")
})
